...
class Qcloud extends BaseDriver
{
    ...
    public function __construct($config = [])
    {
        ...
        $this->cosClient = new CosClient(['region' => $this->region, 'credentials' => ['appId' => $this->appId, 'secretId' => $this->accessKey, 'secretKey' => $this->secretKey]]);
    }
    public function put($localFile, $saveTo)
    {
        ...
        try {
            $saveTo = ltrim($saveTo, '/');
            $res = $this->cosClient->putObject(['Bucket' => $this->bucket, 'Key' => $saveTo, 'Body' => $handle]);
        } catch (\Exception $ex) {
